...
export class AIGameRoom extends Room<MyGameState> {
    ...
    update(deltaTime: number) {
        ...
        if (this.goalScoredThisRound && this.serverBall.isEnabled() && !this.serverBall.isInFall()) {
            ...
            if (Math.abs(ballZ) < ROOM_CONFIG.GOAL_THRESHOLD) {
                ...
                Logger.debug(`[AI] Goal flag reset - ball back in play at z=${ballZ.toFixed(2)}`);
            }
        }
    ...
    private checkForGoal(): void {
        ...
        if (Math.abs(ballZ) > 4.5) {
            Logger.debug(`[AI] checkForGoal: ballZ=${ballZ.toFixed(2)}, gameOver=${this.state.gameOver}, ballLaunched=${this.ballLaunched}, ballEnabled=${this.serverBall.isEnabled()}, goalScored=${this.goalScoredThisRound}, threshold=${ROOM_CONFIG.GOAL_THRESHOLD}`);
        }
        ...
        if (ballZ > ROOM_CONFIG.GOAL_THRESHOLD) {
            Logger.goal(`[AI] Player scores! ballZ=${ballZ.toFixed(2)} > threshold=${ROOM_CONFIG.GOAL_THRESHOLD}`);
            this.goalScoredThisRound = true;
            this.awardPoint(1);
        } else if (ballZ < -ROOM_CONFIG.GOAL_THRESHOLD) {
            Logger.goal(`[AI] AI scores! ballZ=${ballZ.toFixed(2)} < -threshold=${-ROOM_CONFIG.GOAL_THRESHOLD}`);
            this.goalScoredThisRound = true;
            this.awardPoint(2);
        }
    }
    ...
    private awardPoint(playerNum: number): void {
        if (playerNum === 1) {
            ...
            Logger.info(`[AI] Goal! Player scores! Score: ${this.state.player1Score}-${this.state.player2Score}`);
        } else {
            this.state.player2Score++;
            Logger.info(`[AI] Goal! AI scores! Score: ${this.state.player1Score}-${this.state.player2Score}`);
        }
    ...
    private checkForWinner(): void {
        ...
        if (this.state.player1Score >= winningScore) {
            ...
            Logger.gameOver(`[AI] Player wins! (${this.state.player1Score}-${this.state.player2Score})`);
        } else if (this.state.player2Score >= winningScore) {
            this.state.winner = this.state.player2Id;
            this.state.gameOver = true;
            this.serverBall.setGameOver(true);
            this.serverBall.setEnabled(false);
            Logger.gameOver(`[AI] AI wins! (${this.state.player1Score}-${this.state.player2Score})`);
        }
    }
