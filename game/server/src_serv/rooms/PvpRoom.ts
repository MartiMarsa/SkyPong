...
export class PvpRoom extends Room<MyGameState> {
    ...
    onCreate(options: any): void | Promise<any> {
        ...
        this.expirationTimer = setTimeout(() => {
            if (!this.player2Client) {
                Logger.info(`[PvP] Room expired — no opponent joined within ${ROOM_EXPIRATION_MS / 1000}s`);
        ...
        this.onMessage("input", (client, data) => {
            ...
            if (client.sessionId === this.player1Client?.sessionId) {
                this.inputManager.setInput(client.sessionId, { ...data, _playerNum: 1 });
            } else if (client.sessionId === this.player2Client?.sessionId) {
                this.inputManager.setInput(client.sessionId, { ...data, _playerNum: 2 });
            }
    ...
    update(deltaTime: number) {
        ...
        if (this.goalScoredThisRound && this.serverBall.isEnabled() && !this.serverBall.isInFall()) {
            ...
            if (Math.abs(ballZ) < ROOM_CONFIG.GOAL_THRESHOLD) {
                this.goalScoredThisRound = false;
                Logger.debug(`[PvP] Goal flag reset - ball back in play at z=${ballZ.toFixed(2)}`);
            }
        }
    ...
    private checkForGoal(): void {
        ...
        if (Math.abs(ballZ) > 4.5) {
            Logger.debug(`[PvP] checkForGoal: ballZ=${ballZ.toFixed(2)}, gameOver=${this.state.gameOver}, ballLaunched=${this.ballLaunched}, ballEnabled=${this.serverBall.isEnabled()}, goalScored=${this.goalScoredThisRound}, threshold=${ROOM_CONFIG.GOAL_THRESHOLD}`);
        }
        ...
        if (ballZ > ROOM_CONFIG.GOAL_THRESHOLD) {
            Logger.goal(`[PvP] Player 1 scores! ballZ=${ballZ.toFixed(2)} > threshold=${ROOM_CONFIG.GOAL_THRESHOLD}`);
        ...
        if (ballZ < -ROOM_CONFIG.GOAL_THRESHOLD) {
            Logger.goal(`[PvP] Player 2 scores! ballZ=${ballZ.toFixed(2)} < -threshold=${-ROOM_CONFIG.GOAL_THRESHOLD}`);
            this.goalScoredThisRound = true;
            this.awardPoint(2);
            return;
        }
    ...
    private awardPoint(playerNum: number): void {
        if (playerNum === 1) {
            ...
            Logger.info(`[PvP] Goal! ${this.state.player1Name} scores! Score: ${this.state.player1Score}-${this.state.player2Score}`);
        } else {
            this.state.player2Score++;
            Logger.info(`[PvP] Goal! ${this.state.player2Name} scores! Score: ${this.state.player1Score}-${this.state.player2Score}`);
        }
    ...
    onLeave(client: Client, consented: boolean): void | Promise<any> {
        Logger.info(`[PvP] Client left: ${client.sessionId} (consented: ${consented})`);
