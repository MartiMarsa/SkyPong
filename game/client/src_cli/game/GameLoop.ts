...
export class GameLoop {
    ...
    private _update(): void {
        ...
        const collisionDetected = performance.now() - this._lastCollisionAt < CLIENT_TIMING.COLLISION.WINDOW_MS;
        const now = performance.now();

        if (++this._speedUpdateCounter >= NETWORK.SYNC.SPEED_UPDATE_INTERVAL_FRAMES) {
            const elapsed = (now - this._lastSpeedSampleAt) / 1000;
            this._speed = elapsed > 0 ? Vector3.Distance(this._ball.mesh.position, this._lastBallPosition) / elapsed : 0;
